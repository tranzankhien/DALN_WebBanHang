...
<body class="antialiased bg-gray-50">
    ...
    @php
        ...
        // Guard against missing table (e.g., before running migrations)
        if (Schema::hasTable('advarts_url')) {
            $ads = DB::table('advarts_url')->where('active', 1)->orderBy('id')->get();
            if ($ads->isEmpty()) {
                // Fallback: 3 empty placeholders to keep layout
                $ads = collect([
                    (object)['image_url' => null, 'target_url' => '#', 'title' => 'Quảng cáo trống 1'],
                    (object)['image_url' => null, 'target_url' => '#', 'title' => 'Quảng cáo trống 2'],
                    (object)['image_url' => null, 'target_url' => '#', 'title' => 'Quảng cáo trống 3'],
                ]);
            }
            $adsArray = $ads->map(function($a){
                return [
                    'image' => $a->image_url,
                    'url' => $a->target_url ?? '#',
                    'title' => $a->title ?? ''
            ...
            // Table doesn't exist yet: provide placeholders so page renders without DB
            $ads = collect([
                (object)['image_url' => null, 'target_url' => '#', 'title' => 'Quảng cáo trống 1'],
                (object)['image_url' => null, 'target_url' => '#', 'title' => 'Quảng cáo trống 2'],
                (object)['image_url' => null, 'target_url' => '#', 'title' => 'Quảng cáo trống 3'],
    ...
    <div x-data="advertCarousel(@json($adsArray))" x-init="init()" class="w-[70%] mx-auto">
        <div class="relative h-[40px] overflow-hidden rounded-md shadow">
            <div class="flex h-full transition-transform duration-700" :style="`transform:translateX(-${index*100}%);`">
                <template x-for="(a, i) in ads" :key="i">
                    <a :href="a.url" class="w-full h-full flex-shrink-0" target="_blank" rel="noopener">
                        <template x-if="a.image">
                            <img :src="a.image" :alt="a.title" class="w-full h-full object-cover">
                        </template>
                        <template x-if="!a.image">
                            <div class="w-full h-full bg-gray-100 border border-dashed border-gray-300 flex items-center justify-center text-sm text-gray-500">
                                <span x-text="a.title || 'Quảng cáo'">Quảng cáo</span>
                            </div>
                        </template>
                    </a>
                </template>
    ...
    function advertCarousel(initialAds){
        return {
            ...
            timer: null,
            init(){
                // start auto-rotate only if more than 1 ad
                if(this.ads.length > 1){
                    this.start();
                    // pause on hover
                    const el = this.$el.querySelector('.relative');
                    if(el){
            ...
            goTo(i){
                ...
                // reset timer so user has full interval after manual change
                            ...
                            <div
                                ...
                                <div class="relative h-48 bg-gray-100 overflow-hidden">
                                    ...
                                    <div
                                        ...
                                        <button
                                            class="px-4 py-2 bg-white text-gray-900 rounded-lg font-medium transform scale-75 group-hover:scale-100 transition-transform">
                                            Xem nhanh
                                        </button>
                                    </div>
                                </div>
                                ...
                                <div class="p-4">
                                    ...
                                    <h3
                                        class="font-semibold text-gray-900 mb-2 line-clamp-2 group-hover:text-blue-600 transition">
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-5">
            ...
            <div
                ...
                <div class="relative bg-gray-50">
                    ...
                    @if($mainImage)
                    <img src="{{ $mainImage->image_url }}" alt="{{ $product->name }}" class="w-full h-48 object-cover">
                    <a href="{{ route('productInformation', $product->id) }}" class="absolute inset-0"
                        aria-label="Xem {{ $product->name }}"></a>
                    @else
                ...
                <div class="p-3">
                    ...
                    <div class="mt-2">
                        @if($product->discount_price)
                        <div class="text-red-600 font-bold text-base">{{ number_format($product->discount_price) }}đ
                        </div>
                        <div class="text-gray-400 text-xs line-through">{{ number_format($product->price) }}đ</div>
                        @else
                        <div class="text-gray-900 font-bold text-base">{{ number_format($product->price) }}đ</div>
                        @endif
                    </div>
